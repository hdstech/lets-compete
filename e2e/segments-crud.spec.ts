...
// Segments are now managed inline on the round card (no separate segments
// screen), so these assertions target the segment sub-list rendered within
// the round's card. Segments have no lifecycle columns of their own (T8a's
// total-freeze pattern), so — unlike rounds — there's no advancement config
// to assert on, just name/order CRUD, gated to draft events.

test('adding a segment shows it on the round card', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 3, name: 'Segment 1: History' })).toBeVisible()
...
test('editing a segment persists across a reload', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 3, name: 'Segment 1: Science' })).toBeVisible()
...
test('deleting a segment removes it from the round card', async ({ page }) => {
